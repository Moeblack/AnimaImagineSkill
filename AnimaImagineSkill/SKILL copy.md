...
- query不能使用空格分隔多个关键词，一次查询只能查询连续的单词。使用 lookup_codex(query="rape 强") 这样的空格分隔关键词是错误的。应该使用单个连续的关键词，比如 rape 或 强 分别查询，或者使用英文关键词如 torogao、loli。
...
```
composition 字段写：
2koma, split screen, side by side,
left panel [左格视角], right panel [右格视角]
```

**STEP 2 — 定每格内容**

按需在以下字段中，用 `left panel:` / `right panel:` 前缀描述：

| 字段 | 控制对象 |
|---|---|
| `outfit` | 每格服装 |
| `pose_expression` | 每格动作/表情 |
| `environment` | 每格环境/光线 |
...
|---|---|
| `appearance` | 角色外表 |
| `character` / `series` / `artist` | 角色/作品/画师 |
| `quality_meta_year_safe` | 质量+安全标签 |
...
**STEP 1 — 确定角色外表 `appearance`**

两格共用，完全一致。发色、瞳色、体型、肤色。
...
|---|---|
| `outfit` | 日常/禁欲服装，完整穿着 |
| `pose_expression` | 平静、面无表情、中性 |
| `environment` | 日常场景（教室/办公室/灰底） |

举例：
```
outfit: "long-sleeve knit sweater, black pants, fully buttoned"
pose_expression: "standing, arms at sides, expressionless looking at viewer"
```
...
举例：
```
outfit: "same sweater, unbuttoned fully, no bra, breasts out, pants around thighs, vibrator in panties"
pose_expression: "looking down at own body, half-closed eyes, tongue out slightly, blush, heavy breathing"
```
...
```
2koma + split screen + side by side + grey background + white outline
left panel: [禁欲]
right panel: [禁欲之下]
```
